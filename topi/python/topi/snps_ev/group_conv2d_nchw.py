...
"""The template for snps_ev group_conv2d_nchw"""
import tvm
...
autotvm.register_topi_compute(nn.group_conv2d_nchw, ['snps_ev'], 'direct',
                              nn.group_conv2d_nchw.fdefault)

@autotvm.register_topi_compute(nn.group_conv2d_nchw, ['snps_ev'], ['int8'])
def group_conv2d_nchw_snps_ev(cfg, data, kernel, stride, padding, dilation, groups,
                           out_dtype='float32'):
...
def schedule_conv2d_nchw_snps_ev(cfg, outs):
    """TOPI schedule callback of group conv2d for snps_ev
