...
"""Winograd template for snps_ev backend"""
...
def winograd_snps_ev(cfg, data, kernel, strides, padding, dilation, layout, out_dtype, pre_computed):
...
def schedule_winograd_snps_ev(cfg, s, output, pre_computed):
...
@autotvm.register_topi_schedule(schedule_conv2d_winograd_without_weight_transform,
                                ['snps_ev'], ['winograd'])
def schedule_conv2d_winograd_without_weight_transform_snps_ev(cfg, outs):
    """TOPI schedule callback"""
    s = tvm.create_schedule([x.op for x in outs])

    def _callback(op):
        if 'conv2d_nchw_winograd' in op.tag:
            schedule_winograd_snps_ev(cfg, s, op.output(0), pre_computed=True)
...
@nn.conv2d_alter_layout.register(["snps_ev"])
