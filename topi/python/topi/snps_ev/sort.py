...
@argsort.register(["snps_ev"])
def argsort_snps_ev(data, valid_count=None, axis=-1, is_ascend=1, dtype="float32"):
    ...
    if valid_count is not None:
        ...
        out = tvm.extern([data.shape],
                         ...
                         out_buffers=[out_buf],
                         name="argsort_nms_snps_ev",
                         tag="argsort_nms_snps_ev")
    else:
        ...
        out = tvm.extern([data.shape, data.shape],
                         ...
                         name="argsort_snps_ev",
                         tag="argsort_snps_ev")[1]
    return out

@generic.schedule_argsort.register(["snps_ev"])
...
@topk.register(["snps_ev"])
def topk_snps_ev(data, k=1, axis=-1, ret_type="both", is_ascend=False, dtype="int64"):
...
@generic.schedule_topk.register(["snps_ev"])
