...
Target CreateTarget(const std::string& target_name,
  ...
  } else if (target_name == "snps_ev") {
    t->device_type = kDLSNPS_EV;
    t->keys_array.push_back(ir::StringImm::make("snps_ev"));
    //t->keys_array.push_back(ir::StringImm::make("gpu"));
    t->max_num_threads = 64;
    t->thread_warp_size = 32;
...
Target snps_ev(const std::vector<std::string>& options) {
  return CreateTarget("snps_ev", options);
}
